...
namespace Tests.IntegrationTests.Checkout.CheckoutUserTests
{
    [TestClass]
    public class When_proceeding_to_checkout_as_user
    {
        public static Response<CheckoutResponse> _result;
        public static CheckoutUser _testObject;
        ...
        {
            _result.resultset.IsLoggedIn.Should().BeTrue();
        ...
        [TestMethod]
        public void It_should_return_valid_SavedAddresses()
        {
            var savedAddresses = _result.resultset.SavedAddresses;

            savedAddresses.Should().NotBeEmpty();
            foreach (var address in savedAddresses)
            {
                address.Name.Should().NotBeNullOrEmpty();
            }

            savedAddresses.Find(x => x.IsSelected).Should().NotBeNull();
            savedAddresses.Find(x => !string.IsNullOrEmpty(x.Value)).Should().NotBeNull();
            savedAddresses.Find(x => !string.IsNullOrEmpty(x.AccountAddress.Address1)).Should().NotBeNull();
        }
    }
}
