...
namespace Tests.IntegrationTests.Cart.CartApplyCouponTests
{
    [TestClass]
    public class When_getting_applying_coupon
    {
        public static Response<CartResponse> _result;
        public static CartApplyCoupon _testObject;
        ...
        public static void Initialize(TestContext context)
        {
            ...
            var cartApplyCoupon = new BaseIntegrationTest<CartApplyCoupon, CartResponse>();
            var cartApplyCouponRequest = RequestBuilder.GetCartApplyCouponRequest();
            _result = (Response<CartResponse>)cartApplyCoupon.TestObject.Execute(cartApplyCouponRequest);

            _testObject = cartApplyCoupon.TestObject;
        }
        ...
        [TestMethod]
        public void It_should_apply_coupon()
        {
            _result.resultset.Cart.CartItems.Find(x => !string.IsNullOrEmpty(x.TotalPrice.Label)).Should().NotBeNull();
            _result.resultset.Cart.Coupons.Should().NotBeEmpty();
            _result.resultset.Cart.Coupons.FirstOrDefault().Code.Should().NotBeNullOrEmpty();
            _result.resultset.Cart.Coupons.FirstOrDefault().Message.Should().NotBeNullOrEmpty();
            _result.resultset.Cart.Coupons.FirstOrDefault().CouponValue.Value.Should().BeGreaterThan(0);
        }
    }
}
